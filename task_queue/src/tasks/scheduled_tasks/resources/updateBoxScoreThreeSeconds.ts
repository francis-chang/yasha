...
import { getBoxScoreByDateDelta } from '../../../utils/api'
...
const updateGame = async (GameID: number, boxScore: BoxScoreResponse) => {
    return prismaClient.game.update({
        ...
        data: {
            ...
            TimeRemainingMinutes: `${boxScore.Game.TimeRemainingMinutes}`,
            TimeRemainingSeconds: `${boxScore.Game.TimeRemainingSeconds}`,
...
export default async () => {
    ...
    const boxScores = await getBoxScoreByDateDelta(nba_day)
    if (boxScores) {
